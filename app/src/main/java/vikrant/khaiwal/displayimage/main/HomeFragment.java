...
public class HomeFragment extends Fragment{
    ...
    private void getHomePageImages() {
        ...
        Call<MainClass> call = service.getAllImages();
    ...
    private void getHomePaginationImages() {
        RestClient.RestInterface service = RestClient.getClient(getActivity());
        Call<MainClass> call = service.getAllImagePagination(items.size()+1, "pid,large_image_url,title", "json");
        call.enqueue(new retrofit2.Callback<MainClass>() {
            @Override
            public void onResponse(Call<MainClass> call, retrofit2.Response<MainClass> response) {
                if (response != null && response.isSuccessful() && response.errorBody() == null){
